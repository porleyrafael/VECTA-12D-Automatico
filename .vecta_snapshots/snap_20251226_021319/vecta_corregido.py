...
'''
            ...
            tamaño = Config.PAQUETE_PKG.stat().st_size  # CORREGIDO: Usar .stat().st_size
...
'''
        ...
        # Crear ZIP con rutas corregidas
        archivos = [
            Config.PROJECT_DIR / "INSTALAR.bat",
            Config.PROJECT_DIR / "vecta_self_install.py",
            Config.PROJECT_DIR / "vecta_12d_launcher.py",
            Config.PAQUETE_PKG,
        ]
        
        with zipfile.ZipFile(Config.ZIP_FINAL, 'w', zipfile.ZIP_DEFLATED) as zipf:
            for archivo in archivos:
                if archivo.exists():  # CORREGIDO: Usar .exists() en Path
                    zipf.write(archivo, arcname=archivo.name)  # CORREGIDO: .name para solo el nombre
        
        tamaño = Config.ZIP_FINAL.stat().st_size  # CORREGIDO: Usar .stat().st_size
...
def ejecutar_pruebas(diag):
    ...
    try:
        # Prueba 1: Verificar archivos CORREGIDA
        requeridos = [
            Config.DIMENSIONES_DIR / "dimension_1.py",
            Config.DIMENSIONES_DIR / "vector_12d.py",
            Config.CORE_DIR / "vecta_12d_core.py",
            Config.PAQUETE_PKG,   # CORREGIDO: Ahora es Path, no string
            Config.ZIP_FINAL      # CORREGIDO: Ahora es Path, no string
        ]
        ...
        for r in requeridos:
            if r.exists():  # CORREGIDO: .exists() funciona con Path
                existentes.append(r.name)
            else:
                faltantes.append(r.name)
...
def main():
    print("\n" + "="*60)
    print("VECTA 12D - CONSTRUCCION AUTOMATICA (VERSION CORREGIDA)")
    print("="*60)
    ...
    try:
        ...
        if diag.reporte():
            ...
            print(f"\nArchivos creados:")
            print(f"  • {Config.PAQUETE_PKG.name} (paquete interno)")
            print(f"  • {Config.ZIP_FINAL.name} (distribucion)")
